...
def lex(text: str, filename: str="<input>") -> Tuple[List[Token], List[Diagnostic]]:
    ...
    # Normalize newlines, do NOT normalize unicode content
    text = text.replace("\r\n","\n").replace("\r","\n")
    lines = text.split("\n")
    ...
    for li, raw in enumerate(lines, start=1):
        ...
        while i < len(line):
            ...
            # Non-ASCII outside strings is forbidden unless it's an emoji start
            if ord(ch) > 127 and not _is_emoji_start(ch):
                lex_error(li, col, "HND-LEX-0004", f"Non-ASCII character '{ch}' is not allowed outside strings in HAND Core v0.1.", "Move it into a string literal, or replace it.")
                i+=1; col+=1; continue
